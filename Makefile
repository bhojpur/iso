# go tool nm ./isomgr | grep Commit
override LDFLAGS += -X "github.com/bhojpur/iso/cmd/manager.BuildTime=$(shell date -u '+%Y-%m-%d %I:%M:%S %Z')"
override LDFLAGS += -X "github.com/bhojpur/iso/cmd/manager.BuildCommit=$(shell git rev-parse HEAD)"
...
.PHONY: image
image:
	docker build --rm -t bhojpur/base .
...
.PHONY: test-docker
test-docker:
	docker run -v $(ROOT_DIR):/go/src/github.com/bhojpur/iso \
				--workdir /go/src/github.com/bhojpur/iso -ti golang:latest \
				bash -c "make test"
